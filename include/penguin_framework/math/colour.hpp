...
struct PENGUIN_API [[nodiscard]] Colour {
    ...
    constexpr Colour(float p_r, float p_g, float p_b, float p_a) :
        r(std::clamp(penguin::math::compat::p_isnan(p_r) ? 0.0f : p_r, 0.0f, 1.0f)),
        g(std::clamp(penguin::math::compat::p_isnan(p_g) ? 0.0f : p_g, 0.0f, 1.0f)),
        b(std::clamp(penguin::math::compat::p_isnan(p_b) ? 0.0f : p_b, 0.0f, 1.0f)),
        a(std::clamp(penguin::math::compat::p_isnan(p_a) ? 0.0f : p_a, 0.0f, 1.0f)) {
    }

    constexpr Colour(float p_r, float p_g, float p_b) :
        Colour(p_r, p_g, p_b, 1.0f) {} // propagate to regular constructor to handle NaN values

...
TEST(StringConstructorTest, DefaultConstructor) {
	String str;
	EXPECT_EQ(str.c_str(), ""); // the buf is a nullptr, but the c_str should return "" as buf is null
}

TEST(StringConstructorTest, RegularConstructor) {
	String str("Hello world!");
	EXPECT_EQ(str.c_str(), "Hello world!");
}

TEST(StringConstructorTest, CopyConstructor) {
	String original("Copied string!");
	String copy(original);
	EXPECT_EQ(copy.c_str(), "Copied string!");
}

TEST(StringConstructorTest, MoveConstructor) {
	String moved_from("Temp string!");
	String moved_to(std::move(moved_from));
	EXPECT_EQ(moved_to.c_str(), "Temp string!");
	EXPECT_EQ(moved_from.c_str(), ""); // moved-from string should be empty
}
...
TEST(StringAssignmentTest, CopyAssignment) {
	...
	String str2;
	str2 = str1;
	EXPECT_EQ(str2.c_str(), "Hello assignemt!");
}
...
TEST(StringAssignmentTest, MoveAssignment) {
	...
	String str2;
	str2 = std::move(str1);
	EXPECT_EQ(str2.c_str(), "Move me!");
	EXPECT_EQ(str1.c_str(), "");
}
